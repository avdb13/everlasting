...
use crate::{helpers::prettier, writer::MyWriter};

pub struct App {
    actions: StatefulList<String>,
    rx_events: StatefulList<String>,
...
impl App {
    pub fn new(tx: Sender<Action>, tick_rate: Duration, writer: MyWriter) -> Self {
        let titles = vec!["connect".to_owned(), "announce".to_owned()];
        let (rx, tx): (Sender<Vec<String>>, Receiver<Vec<String>>) = mpsc::channel(10);
        ...
        // Self {
        //     actions: StatefulList::new(titles.clone()),
        //     stdout: StatefulList::new(writer),
        //     table: StatefulTable::new(titles),
        //     writer,
        //     tick_rate,
        //     tx,
    ...
    pub async fn run<B: Backend>(&mut self, term: &mut Terminal<B>) -> Result<(), Report> {
    ...
    fn on_tick(&mut self) -> Result<(), Report> {
        ...
        let x = self.writer.queue.remove(0);
        self.writer.queue.push(x);
